...
function AlertRow({ alert }: { alert: AlertHistoryItem }) {
  ...
  return (
    <tr className="border-b hover:bg-gray-50 text-sm">
      ...
      <td className="px-4 py-3 text-gray-500 text-xs">{alert.station?.name}</td>
...
export default function BackofficeAlertsPage() {
  ...
  useEffect(() => {
    stationsApi.getAll().then((s) => setStations(s)).catch(() => {});
  }, []);
  ...
  }, [page, stationFilter, typeFilter, from, to]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">All stations • {total} alert{total !== 1 ? 's' : ''}</p>
          </div>
        </div>
